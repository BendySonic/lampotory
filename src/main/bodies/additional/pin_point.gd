class_name PinPoint
extends Node2D

signal pin_point_connected()
signal pin_point_disconnected()

var all_properties: Array[Dictionary]
var allowed_properties: PackedStringArray = [
		"main_body_path", "parent_body_path",
		"connected_pin_point_path","has_connectable_bodies", "is_connected"
]

@export var main_body_path: NodePath
@export var parent_body_path: NodePath
var connected_pin_point_path: NodePath

var main_body: NormalBody
var parent_body: PhysicsBody2D
var connected_pin_point: PinPoint

var has_connectable_bodies := false
var is_connected := false
	
@onready var pin_joint: PinJoint2D = get_node("PinJoint2D")
@onready var area: Area2D = get_node("Area2D")

func _ready():
	all_properties = get_property_list()
	
	main_body = get_node(main_body_path)
	parent_body = get_node(parent_body_path)
	
	main_body.connect("body_unheld", _on_body_unheld)
	main_body.connect("body_held", _on_body_held)
	
	if (
			main_body.is_loaded and is_connected
			and connected_pin_point == null
			and not connected_pin_point_path.is_empty()
	):
		connected_pin_point = get_node(connected_pin_point_path)
		connect_pin_point(connected_pin_point)

func _physics_process(delta):
	has_connectable_bodies = area.has_overlapping_areas()
	if is_connected and connected_pin_point == null and not connected_pin_point_path.is_empty():
		connected_pin_point = get_node(connected_pin_point_path)
		connect_pin_point(connected_pin_point)

func _on_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.button_index == 1:
			if event.is_pressed():
				main_body.hold_body_with_pin()
	viewport.set_input_as_handled()

func _input(event):
	if event is InputEventMouseButton:
		if event.button_index == 1:
			if not event.is_pressed():
				main_body.unhold_body()

func _on_body_unheld(_body):
	connect_connectable_bodies()

func _on_body_held(_body):
	if not main_body.is_in_group("tripod"):
		if not connected_pin_point_path.is_empty():
			connected_pin_point = get_node(connected_pin_point_path)
		disconnect_body()

func connect_connectable_bodies():
	for area in area.get_overlapping_areas():
		if area.is_in_group("pin_point"):
			var pin_point = area.get_parent()
			if pin_point.is_connected:
				continue
			pin_joint.node_a = pin_joint.get_path_to(parent_body)
			pin_joint.node_b = pin_joint.get_path_to(pin_point.get_body())
			connected_pin_point = pin_point
			connected_pin_point_path = connected_pin_point.get_path()
			is_connected = true
			
			connected_pin_point.connected_pin_point_path = connected_pin_point.get_path_to(self)
			
			pin_point.emit_signal("pin_point_connected")
			emit_signal("pin_point_connected")
			
			break

func connect_pin_point(pin_point: PinPoint):
	if not pin_point == null:
		pin_joint.node_a = pin_joint.get_path_to(parent_body)
		pin_joint.node_b = pin_joint.get_path_to(pin_point.get_body())

func disconnect_body():
	if not connected_pin_point == null:
		connected_pin_point.clear_pin_joint()
	clear_pin_joint()
	is_connected = false

func clear_pin_joint():
	pin_joint.node_a = ""
	pin_joint.node_b = ""
	connected_pin_point_path = NodePath("")
	connected_pin_point = null
	is_connected = false

func get_body():
	return parent_body

func get_main_body():
	return main_body

func get_all_properties():
	var result: Dictionary
	
	for property in all_properties:
		if allowed_properties.has(property["name"]):
			var value = get(property["name"])
			if value is Dictionary:
				result[property["name"]] = value.duplicate()
			else:
				result[property["name"]] = value
	
	return result
