...
@onready var workspace_instance = (preload
		("res://resources/scenes/workspace/workspace.tscn").instantiate())


func _on_label_1d_gui_input(event):
	open_workspace(MODE.ONE_MECHANIC)


func _on_label_2d_gui_input(event):
	open_workspace(MODE.TWO_MECHANIC)


func open_workspace(mode:int):
